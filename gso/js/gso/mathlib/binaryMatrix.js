...
;(function () {
  ...
  Gso.MathLib.BinaryMatrix = function BinaryMatrix () {
    ...
    this.duplicateMatrix = function () {
      var newOne = Gso.MathLib.BinaryMatrix.newOne(this.elements)
      return newOne
    }
    ...
    this.isSameSizeAs = function (matrix) {
      ...
      if (typeof (tempMatrix[0][0]) === 'undefined') {
        tempMatrix = Gso.MathLib.BinaryMatrix.new_one(tempMatrix).elements
      }
      if (this.elements.length === 0) {
    ...
    this.mapProcess = function (procFunction, context) {
      if (this.elements.length === 0) {
        return Gso.MathLib.BinaryVector.newOne([])
      }
      var elements = []
      ...
      while (numberOfRows--) {
        ...
        while (tempNumberOfColumns--) {
          elements[numberOfRows][tempNumberOfColumns] = procFunction
              .call(context,
          /* changed numberOfColumns to tempNumberOfColumns */
    ...
    this.boolMultiply = function (binaryMatrix) {
      ...
      var newMatrix = Gso.MathLib.BinaryMatrix.newOne(elements)
    ...
    this.matrixView = function () {
      ...
      for (var row = 0; row < elementsLength; row++) {
        matrixRows.push(Gso.MathLib.BinaryVector.newOne(this.elements[row]).view())
      }
      return matrixRows.join('<br>')
    }
  }

  Gso.MathLib.BinaryMatrix.newOne = function (elements) {
